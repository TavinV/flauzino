...
/**
 * Marca do produto: águia da Flauzino (placeholder white-label temporário)
 * com wordmark "Visage". Path extraído de design-system/assets/flauzino-logo.svg.
 * EagleMark é exportado para usos de marca fora do lockup (ex.: watermark
 * "Flauzino" no overlay da câmera do totem).
 */
export function EagleMark({ size }: { size: number }) {
  const h = size * (257 / 361);
  return (
    <svg
      width={size}
      height={h}
...
export function Logo({ variant = "lockup", size = 40, tagline, className }: LogoProps) {
  if (variant === "mark") {
    return (
  ...
  return (
    <span
      ...
      style={{ gap: stacked ? size * 0.25 : size * 0.32 }}
    >
      <EagleMark size={size} />
      <span
        className={cn("flex flex-col leading-none", stacked ? "items-center" : "items-start")}
      >
        <b
        ...
        {tagline && (
          <small
            className="mt-1 font-medium uppercase opacity-70"
            style={{ fontSize: Math.max(9, size * 0.22), letterSpacing: "0.18em" }}
          >
            {tagline}
          </small>
        )}
