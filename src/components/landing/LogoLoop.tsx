...
/* ================================================================== */
/*  LogoLoop — porte do componente reactbits.dev/animations/logo-loop. */
/*  A faixa não usa keyframes CSS: um rAF integra velocidade em px/s   */
/*  e escreve translate3d no track, o que deixa o hover desacelerar    */
/*  suave em vez de travar de uma vez. O número de cópias da sequência */
/*  é medido por ResizeObserver, então a emenda nunca aparece, seja    */
/*  qual for a largura da tela. Respeita prefers-reduced-motion.       */
/*  Únicas adaptações ao original: "use client" (App Router) e         */
/*  scale-110 no hover — scale-120 não existe no Tailwind 3.           */
/* ================================================================== */
...
export const LogoLoop = React.memo<LogoLoopProps>(function LogoLoop({
  ...
  const renderLogoItem = useCallback(
    (item: LogoItem, key: React.Key) => {
      ...
      if (renderItem) {
        return (
          <li className={liClasses} key={key} role="listitem">
            {renderItem(item, key)}
          </li>
        );
      }
      ...
      const content =
        "node" in item ? (
          <span
            className={cx("inline-flex items-center motion-reduce:transition-none", scaleClasses)}
      ...
      {fadeOut &&
        (isVertical ? (
          ...
          <>
            <div
              aria-hidden
              className="pointer-events-none absolute inset-y-0 left-0 z-10 w-[clamp(24px,10%,180px)] bg-[linear-gradient(to_right,var(--logoloop-fadeColor,var(--logoloop-fadeColorAuto))_0%,rgba(0,0,0,0)_100%)]"
            />
            <div
              aria-hidden
              className="pointer-events-none absolute inset-y-0 right-0 z-10 w-[clamp(24px,10%,180px)] bg-[linear-gradient(to_left,var(--logoloop-fadeColor,var(--logoloop-fadeColorAuto))_0%,rgba(0,0,0,0)_100%)]"
            />
          </>
        ))}
